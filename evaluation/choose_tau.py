"""Choose Tau - Optimal threshold selection for LAT probe."""
import json
...
def load_scores(path: str) -> Tuple[np.ndarray, np.ndarray]:
    """Load scores and labels from JSONL."""
    scores, labels = [], []
    with open(path) as f:
        for line in f:
...
def find_optimal_tau(
    ...
    if len(valid) > 0:
        ...
        candidates.append({"name": f"min_safety_{int(min_safety*100)}", **compute_metrics_at_tau(scores, labels, tau_safe)})
    
    # Percentile-based
    harm_scores = scores[labels == 1]
    for p in [5, 10]:
        tau_p = np.percentile(harm_scores, p)
        candidates.append({"name": f"harmful_p{p}", **compute_metrics_at_tau(scores, labels, tau_p)})
    
    return {"auc": float(auc), "candidates": candidates}


def print_tau_analysis(analysis: Dict):
    """Pretty print tau analysis."""
    print(f"\nAUC: {analysis['auc']:.4f}")
    print("-" * 60)
    print(f"{'Name':<20} {'Tau':>8} {'Safety':>10} {'Comply':>10} {'Trade':>10}")
    print("-" * 60)
    for c in analysis["candidates"]:
        print(f"{c['name']:<20} {c['tau']:>8.4f} {c['safety_rate']:>10.2%} "
